...
## Four Operational Loops

| Loop | When to use | Skill to load |
|------|-------------|---------------|
| **Ingest** | Adding a new source document to the knowledge base | `ikai-ingest-workflow` |
| **Connect** | After ingest, link new nodes; or on demand to discover missed connections | `ikai-connect-workflow` |
| **Query** | Answering a question from the graph | `ikai-query-workflow` (uses LM tools — no terminal commands) |
| **Maintenance** | Health-checking the graph — stale nodes, orphans, backlinks | `ikai-maintenance-workflow` |
...
Before any operation, read `.ikai/config.yaml` in the workspace root to find the active namespace. Never guess or default to `@owner/project`.
...
If `.ikai/config.yaml` is missing, ask the developer to run `ikai init <name>` first.
...
<description>REQUIRED for running an ikai ingest session. Use when adding a source document to the knowledge base. Triggers: ikai ingest, ingest session, add source, process article, add to wiki, add to knowledge base, ingest this file. Excludes query, maintenance, and connection-finding operations.</description>
<file>.github/skills/ikai-ingest-workflow/SKILL.md</file>
</skill>
<skill>
<name>ikai-query-workflow</name>
<description>REQUIRED for answering questions from the ikai knowledge base. Use when a developer asks a question from the graph. Triggers: ikai traverse, query wiki, what do I know about, search my wiki, find connections between, answer from graph, what does the wiki say. Excludes ingest, maintenance, and connection-finding operations.</description>
<file>.github/skills/ikai-query-workflow/SKILL.md</file>
</skill>
<skill>
<name>ikai-connect-workflow</name>
<description>REQUIRED for finding and creating connections between nodes in the knowledge graph. Use after ingest to link new nodes, or on demand to discover missed connections. Triggers: connect nodes, find connections, link documents, cast the net, cross-namespace connections, connect these documents, relate nodes, missed links, what connections am I missing, link up the graph. Excludes ingest (document processing), query (answering questions), and structural health (stale/orphan/backlink).</description>
<file>.github/skills/ikai-connect-workflow/SKILL.md</file>
</skill>
<skill>
<name>ikai-maintenance-workflow</name>
<description>REQUIRED for running an ikai maintenance session. Use when health-checking the graph or resolving stale/orphan/backlink issues. Triggers: ikai lint, maintenance session, health check, stale nodes, orphan nodes, backlinks, clean up wiki, fix graph, graph health. Excludes ingest, query, and connection-finding operations.</description>
